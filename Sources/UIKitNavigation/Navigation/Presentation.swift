...
extension UIViewController {
  ...
  //       Maybe not useful and would also pose an issue with ambiguity when only one is specified.
  ...
  ///     non-`nil`, the item's content is passed to the `content` closure. You display this content
  ///     in a view controller that you create that is displayed to the user. If `item`'s identity
  ///     changes, the view controller is dismissed and replaced with a new one using the same
  ///     process.
  ///   - onDismiss: The closure to execute when dismissing the view controller.
  ///   - content: A closure that returns the view controller to display over the current view
  ///     controller's content.
